export function drawDarkParallax(
  ctx,
  canvas,
  parallaxLayers,
  groundY
...
export function drawModernCity(ctx, x, y, height) {
  ...
  for (const building of buildings) {
    ...
    for (
      let wy = buildingY + 10 * scale;
      wy < buildingY + building.height - 5 * scale;
      wy += windowHeight + verticalSpacing
    ) {
      // Alternating window patterns for modern look
      const offset =
        (wy / (windowHeight + verticalSpacing)) % 2 === 0
          ? 0
          : horizontalSpacing / 2;

      for (
        let wx = buildingX + 5 * scale + offset;
        wx < buildingX + building.width - windowWidth;
        wx += windowWidth + horizontalSpacing
      ) {
        // Add some variation
        if (Math.random() > 0.2) {
...
export function drawLightParallax(
  ctx,
  canvas,
  parallaxLayers,
  groundY
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 50; j++) {
      ...
      const height =
        layer.hedgeHeights && layer.hedgeHeights[j]
          ? layer.hedgeHeights[j]
          : 15;
      ctx.beginPath();
...
export function drawCitySilhouette(ctx, x, y, height) {
  ...
  for (const building of buildings) {
    ...
    for (
      let wy = y + height - building.height + 10 * scale;
      wy < y + height - 5 * scale;
      wy += windowHeight + 5 * scale
    ) {
      for (
        let wx = x + building.x + 5 * scale;
        wx < x + building.x + building.width - windowWidth;
        wx += windowWidth + spacing
      ) {
        ctx.fillRect(wx, wy, windowWidth, windowHeight);
      }
    }
...
export function updateParallax(parallaxLayers, canvas) {
  for (let i = 0; i < parallaxLayers.length; i++) {
    ...
    if (i === 3) {
      parallaxLayers[i].offset =
        (parallaxLayers[i].offset - parallaxLayers[i].speed * 3.0) %
        canvas.width; // 20% faster than 2.5
    } else {
      parallaxLayers[i].offset =
        (parallaxLayers[i].offset - parallaxLayers[i].speed * 1.8) %
        canvas.width; // 20% faster than 1.5
    }
  }
}
